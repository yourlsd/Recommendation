# -*- coding: utf-8 -*-
"""
Created on Sun Sep 09 20:58:37 2018
...
af = pd.read_csv("C:/Users/slu/Desktop/Recommendation/Article Recommendation/Sample Data/Affinity9_17.csv", header = None)
pp = pd.read_csv("C:/Users/slu/Desktop/performance and parent9-17.csv")
ca = pd.read_csv("C:/Users/slu/Desktop/Recommendation/Article Recommendation/Sample Data/Calculators8_9.csv")
#tr = pd.read_csv('C:/Users/slu/Desktop/textrectext.csv')

def get_recommendations(id, data, n_text):
    
    tfidf = TfidfVectorizer(stop_words='english')

    data['_yoast_wpseo_metadesc'] = data['_yoast_wpseo_metadesc'].fillna('')

    tfidf_matrix = tfidf.fit_transform(data['_yoast_wpseo_metadesc'])
    ...
    sim_scores = sim_scores[1: n_text+1]

    article_indices = [i[0] for i in sim_scores]

    result = data['id'].iloc[article_indices].as_matrix()
...
for index, row in pp.iterrows():
    ...
    try:
        ...
        # calculate a list for text
        list_text = get_recommendations(row['id'], pp , n_text)
        
        aff_text_dup = np.intersect1d(list_text, list_affinity)
        
        list_text = np.setdiff1d(list_text,aff_text_dup)
        
        
        #print row['id']
        group_remain = group_remain[~group_remain['id'].isin(list_text)]
        ...
        plist_random = []
        n_random = n_total - len(list_affinity) - len(list_popular) - len(list_calculator) - len(list_text)
        group_remain_length = len(group_remain)
        ...
        if n_random-len(plist_random) > 0:
            ...
            sgroup_remain = sgroup_remain[~sgroup_remain['id'].isin(list_text)]
        ...
        pp.loc[pp['id'].isin(list_text), 'count'] += 1

        # write row to file
        recommendations = str(list_affinity.astype(int)) + ',' + str(list_popular.astype(int)) + ',' + str(list_text.astype(int)) + ',' + str(plist_random.astype(int)) + ',' + str(slist_random.astype(int)) + ',' + str(list_calculator.astype(int))
        rec = map(int,list_affinity.tolist() + list_popular.tolist() + list_text.tolist() + plist_random.tolist() + slist_random.tolist() + list_calculator.tolist())
        
        f.write('%s,"%s","%s","%s"\n' % (str(int(row['id'])), row['Title'], recommendations,rec))
